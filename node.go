...
func init() {
	...
	error := json.Unmarshal([]byte(readFile(addrStr)), &addresses)
	if error != nil {
...
func chainNew(filename string) *bc.BlockChain {
	error := bc.NewChain(filename, User.Address())
	if error != nil {
		return nil
	}
...
func addBlock(pack *nt.Package) string {
	...
	if!block.IsValid(Chain, Chain.Size()) {
		currSize := Chain.Size()
		num, error := strconv.Atoi(splited[1])
		if error != nil {
			return "fail"
		}
...
func addTransaction(pack *nt.Package) string {
	...
	Mutex.Lock()
	error := Block.AddTransaction(Chain, tx)
	Mutex.Unlock()
	if error != nil {
		return "fail"
	}
...
func getBlock(pack *nt.Package) string {
	num, error := strconv.Atoi(pack.Data)
	if error != nil {
		return ""
	}
	size := Chain.Size()
...
func compareChains(address string, num uint64) {
	...
	defer db.Close()
	_, error = db.Exec(bc.CREATE_TABLE)
	chain := &bc.BlockChain {
		DB : db,
	}
